;(function(){
  ...
  function openComplaintModal(rideId, driverId, driverName) {
      ...
      cc && cc.classList.remove('warning', 'error');
  ...
  async function submitComplaint() {
      ...
      if (!complaintText) { showComplaintMessage('Please describe what happened.', 'error'); return; }
      if (complaintText.length < 10) { showComplaintMessage('Please provide more details (at least 10 characters).', 'error'); return; }
      ...
      submitBtn.disabled = true; loadingEl.classList.add('active'); hideComplaintMessage();
      try {
          ...
          if (response.ok && result.success) {
              ...
              document.getElementById('complaintText').value = '';
              setTimeout(() => { closeComplaintModal(); }, 2000);
          } else {
          ...
          submitBtn.disabled = false; loadingEl.classList.remove('active');
      }
  }
  ...
  function hideComplaintMessage() { const messageEl = document.getElementById('complaintMessage'); messageEl.style.display = 'none'; }

  document.addEventListener('keydown', (e) => {
      if (e.key === 'Escape') {
          const modal = document.getElementById('complaintModal');
          if (modal && modal.style.display === 'block') closeComplaintModal();
      }
  });
